...
Let $\tsymbs \defeq \{ t', t'', t''' ... \}$ be a set of items we call
...
\begin{defn}
    The function $ts : \tclos{\tsymbs} \rightarrow 2^\tsymbs$, which
    returns the set of type symbols used in a complex type, is defined inductively:

    \begin{itemize}
        \item if $\sigma \in \tsymbs$ then $ts(\sigma) = \sigma$
...
Let $\lvars = \{ \alpha', \alpha'', \alpha''' ... \}$ be a countable set of items
we call \emph{variables} and $\const = \{ c', c'', c''' ... \}$ be a countable
set of items we call \emph{constants}.
