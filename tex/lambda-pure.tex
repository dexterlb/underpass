...
\begin{defn}
    We call a set $X$ \emph{typed in} $T$ when there is a function\fixednote{\scriptsize $\tot$ се използва в един и същ контекст с две различни значения}
    \[ \typeoff : X \fun \tclos{T} \]
...
Now, we will prove that every well-typed term has a unique type:\discuss{\scriptsize формално погледнато, преди това трябва да докажеш Лема за обръщането, иначе с индукция по $M$ не можеш да правиш заключения зи $\vdash$}
\begin{prop}
