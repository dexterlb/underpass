...
For iterative derivation, we will use strings in $(\lb \cclos{\tau} \rb \cup \Sigma)^*$,\marginpar{\scriptsize Няма ли да е удобно да въведеш нотация за множеството $(\lb \cclos{\tau} \rb \cup \Sigma)^*$? Например $\tau^\star$ или нещо такова.}
...
\begin{defn}
    ...
    finite alphabet of terminals, we define $\cclos{\alpha}$\marginpar{\scriptsize $\cclos{\cdot}$ е конфликт на нотации} to be the set of all
    categories that
