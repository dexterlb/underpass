...
For iterative derivation, we will use strings in $S(\tau)$,\fixednote{\scriptsize Няма ли да е удобно да въведеш нотация за множеството $(\lb \cclos{\tau} \rb \cup \Sigma)^*$? Например $S(\tau)$ или нещо такова.}
...
\begin{defn}
    ...
    finite alphabet of terminals, we define $\catsin{\alpha}$\fixednote{\scriptsize $\catsin{\cdot}$ е конфликт на нотации} to be the set of all
    categories that
