...
\marginpar{\tiny Не е ли по-добре \code{<simple-type>}
...
Note that Minipass is characterised by its predefined constants and basic\marginpar{\scriptsize по-долу използваш base, избери си едното}
...
already defined.\marginpar{\scriptsize необходимо ли е да казваме, че ``\code{NewType} is not a supertype of \code{OldType}'' след като казваме, че ``has not been already defined?''}
