...
Note that Minipass is characterised by its predefined constants and base\fixednote{по-долу използваш base, избери си едното}
...
\end{center}
where \code{NewType} is not a basic type that is a supertype of \code{OldType} 
and has not been
already user-defined.\fixednote{необходимо ли е да казваме, че ``\code{NewType} is not a supertype of \code{OldType} - A: да: може да е вграден'' след като казваме, че ``has not been already defined?''}
