...
\begin{defn}
    ...
    denote potentually complex categories (produced by an arbitrary combination of tjw rules above).
    ...
    doesn't matter). The two slashes will also be considered left-associative.
    ...
    In what follows, for any given set $\tau$ we will consider the
    (potentially infinite) alphabet in which categories are considered
    as letters. To avoid syntactic ambiguity, when categories are used
    as lettets we will enclose them in square brackets: $\lb X
    \rb$. We will denote this alphabet as $\lb C(\tau) \rb$.

    For any string $\alpha \in (\lb C(\tau) \rb \cup \Sigma)^*$, where $\Sigma$ is a
    finite set, we define $C(\alpha)$\footnote{Clash of notation with categorial closure} to be the set of all categories which
...
\begin{defn}
    Breadth\footnote{Length usually denotes the total number of symbols, while here you want the number of arguments instead. Have you considered ``arity''?} of category: for $X = A \mc X_1 \mc X_2 \mc ... \mc X_n \in C(\tau)$,
    $|X| = n$.
\end{defn}
