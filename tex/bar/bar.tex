...
\begin{defn}
    ...
    If a string $ \alpha \in (\lb C(N) \rb \cup \Sigma)^* $ can be derived
    from a category $ X \in C(N) $ with a derivation tree, it shall be denoted
    as $ \lb X \rb \Rrightarrow \alpha $. Formally, $ \lb X \rb \Rrightarrow
    \alpha $ if there exists a derivation tree for $ \lb X \rb \Rrightarrow
    \alpha $.

    Derivation trees are constructed as follows:

    \begin{itemize}
        \item If $ a \in \Sigma $, then $ a $ is a derivation tree for $ a \Rrightarrow a$
            (trivial terminal case).
        \item If $ X \in C(N) $ then $ X $ is a derivation tree for $ \lb X \rb \Rrightarrow \lb X \rb $
            (trivial nonterminal case).
        \item If $ a \in \Sigma, X \in f(a) $, then
            \centree{.$X$ \edge[very thick]; $a$ }
            is a derivation tree for $ \lb X \rb \Rrightarrow a $
        \item If
            \begin{center}
                \tree{.{$X \rc Y$} \edge[roof]; $\alpha$}
                and
                \tree{.{$Y \mc_1 Z_1 \mc_2 Z_2 ... \mc_m Z_m$} \edge[roof]; $\beta$}
            \end{center}

            are derivation trees for $\lb X \rc Y \rb \Rrightarrow \alpha$ and
            $\lb Y \mc_1 Z_1 \mc_2 Z_2 ... \mc_m Z_m \rb \Rrightarrow \beta$ respectively,
            where $m \leq n, \alpha \in \Sigma^*, \beta \in \Sigma^*$, then
            \centree{.{$X \mc_1 Z_1 \mc_2 Z_2 ... \mc_m Z_m$}
                \edge[very thick];
                [ .{$X \rc Y$} \edge[roof]; $\alpha$ ]
                [ .{$Y \mc_1 Z_1 \mc_2 Z_2 ... \mc_m Z_m$} \edge[roof]; $\beta$ ]
            }
            is a derivation tree for $ \lb X \mc_1 Z_1 \mc_2 Z_2 ... \mc_m Z_m \rb
            \Rrightarrow \alpha \beta $.
        \item If
            \begin{center}
                \tree{.{$X \lc Y$} \edge[roof]; $\alpha$}
                and
                \tree{.{$Y \mc_1 Z_1 \mc_2 Z_2 ... \mc_m Z_m$} \edge[roof]; $\beta$}
            \end{center}

            are derivation trees for $\lb X \lc Y \rb \Rrightarrow \alpha$ and
            $\lb Y \mc_1 Z_1 \mc_2 Z_2 ... \mc_m Z_m \rb \Rrightarrow \beta$ respectively,
            where $m \leq n, \alpha \in \Sigma^*, \beta \in \Sigma^*$, then
            \centree{.{$X \mc_1 Z_1 \mc_2 Z_2 ... \mc_m Z_m$}
                [ .{$Y \mc_1 Z_1 \mc_2 Z_2 ... \mc_m Z_m$} \edge[roof]; $\beta$ ]
                \edge[very thick];
                [ .{$X \lc Y$} \edge[roof]; $\alpha$ ]
            }
            is a derivation tree for $ \lb X \mc_1 Z_1 \mc_2 Z_2 ... \mc_m Z_m \rb
            \Rrightarrow \beta \alpha $.
    \end{itemize}
...
\begin{defn}
    We say $\lb X \mc^* Y \rb \Rrightarrow \alpha \lb X \mc^* Z \rb \beta$ is a
    \emph{primary derivation} from $X \mc^* Y$ to $X \mc^* Z$ with prefix $ X $ if:
    \begin{itemize}
        \item $\lb X \mc^* Y \rb \Rrightarrow \alpha \lb X \mc^* Z \rb \beta$
        \item The path in the tree from the root $ X \mc^* Y $
            to the leaf $ X \mc^* Z $
            consists of only primary edges.
        \item Every category in the path looks like $ X \mc^* T $ for some
            $ T \in C(N) $
    \end{itemize}

    (here $ X, Y, Z \in C(N), \mc^* \in \{ \lc, \rc \} $)
\end{defn}
