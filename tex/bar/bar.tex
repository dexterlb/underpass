...
\begin{defn}
    ...
    \begin{itemize}
        \item If $ a \in \Sigma, X \in f(a) $, then
            ...
            is a derivation tree for $ \lb X \rb \Rightarrow^* a $
        \item If
            \begin{center}
            ...
            are derivation trees for $\lb X \rc Y \rb \Rightarrow^* \alpha$ and
            $\lb Y \mc_1 Z_1 \mc_2 Z_2 ... \mc_m Z_m \rb \Rightarrow^* \beta$ respectively,
        ...
        \item If
            ...
            are derivation trees for $\lb X \lc Y \rb \Rightarrow^* \alpha$ and
            $\lb Y \mc_1 Z_1 \mc_2 Z_2 ... \mc_m Z_m \rb \Rightarrow^* \beta$ respectively,
            where $m \leq n, \alpha \in \Sigma^*, \beta \in \Sigma^*$, then
            \centree{.{$X \mc_1 Z_1 \mc_2 Z_2 ... \mc_m Z_m$}
                [ .{$Y \mc_1 Z_1 \mc_2 Z_2 ... \mc_m Z_m$} \edge[roof]; $\beta$ ]
...
\begin{prop}
    ...
    $ \lb X \rb \Rightarrow^* \alpha \iff $ there exists a derivation tree for
    $ \lb X \rb \Rightarrow^* \alpha $
\end{prop}
\begin{proof}
    First, let there exist a derivation tree for $ \lb X \rb \Rightarrow^* \alpha $.
    ...
    be a derivation for $G$. Ignoring the trivial case of reflexivity,
    we have $ \lb X \rb \rightarrow \beta $.
    \begin{itemize}
        \item $\beta = a \in \Sigma, X \in f(a)$. Then $\alpha = \beta$ and
            \centree{.$X$ \edge[very thick]; $a$ }
            is a derivation tree for $ \lb X \rb \Rightarrow^* a = \alpha $
