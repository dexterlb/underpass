        ...
        \item It preserves the subtyping relation:
            ...
            \begin{itemize}
                ...
                \item $\sigma \in (T' \setminus T), \tau \in (T' \setminus T)$:
                    ...
                    $\sigma \less' \sigma'$ \note{only -> least} and $\tau'$ is the only element in $T$
...
\begin{prop}\label{prop:makesquashfun}
    ...
    in $T$.\note{уточнение за minimal и maximal}
\end{prop}
\begin{proof}
    Let \[
...
\begin{example}
    ...
    more concrete types:
    \centree{.{$\langle plus, Num \tot Num \rangle$}
        [ .{$\langle x, Num \rangle $} ]
        [ .{$\langle plus, Num \tot Num \rangle $}
            [ .{$\langle 3, Num \rangle $} ]
            [ .{$\langle 5, Num \rangle $} ]
        ]
    }
