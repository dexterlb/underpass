...
\begin{defn}
    ...
    \begin{itemize}
        ...
        \item Construction via downcast (dangerous, doesn't need to be included in most cases\discuss{\scriptsize не видях в текста никъде да използваш downcast, ако наистина е така, може би е по-добре да го махнеш}            \cenderiv{
                \hypo{\Gamma \vdash A : \sigma}
                \hypo{\tau \less \sigma}
                \infer2{\Gamma \vdash \tau[A] : \tau}
            }
...
\begin{defn}
    ...
    \begin{itemize}
        ...
        \item a mapping $\varphi : \tsymbs \pfun \tclos{T}$\marginpar{\scriptsize тук означението $\pfun$ се появява за пръв път без да е обяснено, не мисля че може да се счита стандартно означение на частична функция.}
...
\begin{lemma}
    ...
    If $\langle T, \less \rangle$ is a type lattice, $\theta$\fixednote{\scriptsize има конфликт на символите $\sigma$ и $\tau$ в $\theta$ с тези по-надолу. Доколкото виждам, никъде не използваш конкретните $\sigma$ и $\tau$, така че може просто да не разписваш $\theta$}
    is a subtype assertion satisfying the conditions from \cref{def:subadd}
    and $\langle T', \less' \rangle = \langle T, \less \rangle \tadd \theta$, then
    there is a unique squash function $\squash_{\theta} : T' \rightarrow T$
    that maps general elements in $T'$ into
...
\begin{proof}
    ...
    $\suptyper{\xi}$ are injections with non-overlapping ranges.\fixednote{\scriptsize и защото range-овете им не се пресичат}
        ...
        \item it preserves the subtyping relation:
            ...
            \begin{itemize}
                ...
                \item $\sigma \in (T' \setminus T), \tau \in (T' \setminus T)$:
                    ...
                    $\sigma \less' \sigma'$ \marginpar{\scriptsize че $\sigma'$ е единствен е ясно, но откъде следва че релацията е $\sigma\less'\sigma'$, а не $\sigma'\less'\sigma$?} and $\tau'$ is the only element in $T$
                    such that $\tau' \less' \tau$, we must have
                    $\sigma \less' \sigma' \less \tau' \less \tau$.
            \end{itemize}
    \end{itemize}

    The only thing left is to check that $\squash_{\theta}$ returns maximal
    elements in $T$ for general elements in $T'$ and returns minimal elements
    in $T$ for special elements in $T'$:

    \begin{itemize}
        \item for each special $\tau$, $\squash_\theta(\tau)$ is a minimal element in $T$ such that
            $\tau \less' \squash_\theta(\tau)$:\fixednote{\scriptsize minimal и (minimal \ldots such that \ldots) са различни свойства!}

            Since $\tau$ is special, it means that $\tau \in (T' \setminus T)$ and
            $\tau \less' \sigma$ for some $\sigma \in T$.

            The only possibility is that $\subtyper{\xi}(\sigma) = \tau$ and
            $\squash_{\theta}(\tau) = \sigma$. But $\sigma$ is the only element
            in $T$ such that $\tau \less' \sigma$, therefore it is minimal
            with this property.\fixednote{\scriptsize ако тук не напишеш ``\ldots minimal with this property'', не мисля, че ще е вярно}
...
\begin{prop}\label{prop:makesquashfun}
    ...
    $\squash_{\Theta} : T' \rightarrow T$ that maps general elements in $T'$ into
    maximal elements in $T$ and maps special elements in $T'$ into minimal elements
    in $T$.
\end{prop}
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item for $T_{i + 1}$, inductively we have that $T_i \tless T$.\fixednote{\scriptsize тук може би искаш да кажеш, че $T_i \tless T$? Нещо не разбирам означенията\ldots като ги обсъдим, ще прегледам пак.}
...
% TT: reviewed until here

\subsubsection{Type-tagged terms and type propagation}
\label{sec:propagation}\marginpar{\scriptsize струва ми се, че този раздел има нужда от мотивиращ пример}
...
process until\marginpar{\scriptsize възможно ли е този процес да зацикли?} it no longer produces a different term, and then $\mcf{untag}$ging
...
}\marginpar{\scriptsize Тази фигура няма ли да се опрости, ако махнеш $\varepsilon$-преходите?}

This construction\marginpar{\scriptsize моля поясни: tagging? manipulation? propagation?} does not need to include casts, since we are storing
...
\begin{defn}
    ...
    The set of type-tagged $\lambda$-terms $\ttlambda{T}{C}$ is defined inductively\marginpar{\scriptsize стандартната нотация за типове по термовете май е $M^\tau$. Ако нарочно въвеждаш различна нотация, за да разграничиш type-tagging от експлицитни системи стил Church, добре е да го кажеш явно.}:
    \begin{itemize}
        \item Constant:
...
describe a universal way to propagate information through those types.\marginpar{\scriptsize няма ли да покажеш, че термовете, които се връщат от $\mcf{untag}$ са коректно типизирани?}

We will use a basic operation $\unify$ that unifies two types.

\begin{defn}
An operation $ \unify : \tclos{T} \times \tclos{T} \pfun \tclos{T}$ is
called a \emph{unifier} for $T$ if:\marginpar{\scriptsize защо не опишеш свойствата формално? мотивиращ пример за унификатор?}
\begin{itemize}
...
\begin{defn}
 ...
 ,& $A = (PQ), P = \ttt{M}{\sigma \tot \eta}, Q = \ttt{N}{\sigma'},$ \\
  ...
  \end{defn}
  \newpage
  \marginpar{\scriptsize липсва стъпката за манипулиране на терм, може би е добре да дадеш поне един пример за такава функция? тук може би е редно да навържеш всички дефинирани функции в една обща функция, която работидиректно над $\lambda$-термове.}
\end{document}
