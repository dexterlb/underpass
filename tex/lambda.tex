...
\begin{defn}
    ...
    \begin{itemize}
        ...
        \item Construction via downcast (dangerous, doesn't need to be included in most cases\marginpar{\scriptsize не видях в текста никъде да използваш downcast, ако наистина е така, може би е по-добре да го махнеш}            \cenderiv{
                \hypo{\Gamma \vdash A : \sigma}
                \hypo{\tau \less \sigma}
                \infer2{\Gamma \vdash \tau[A] : \tau}
            }
...
\begin{defn}
    ...
    \begin{itemize}
        ...
        \item a type $\tau$ such that $\hat{\varphi}(\tau)$ is defined\marginpar{\scriptsize ако започваш от $\varphi = \emptyset$ това условие няма как да е изпълнено}
    \end{itemize}
...
\begin{defn}
    ...
    is defined\marginpar{\scriptsize тук не става ясно, че освен понятието ``subtype library'' дефинираш също и $\hat{\Theta}_T$} \footnote{
...
\begin{lemma}
    ...
    If $\langle T, \less \rangle$ is a type lattice, $\theta = (\sigma \lass \tau)$\marginpar{\scriptsize има конфликт на символите $\sigma$ и $\tau$ в $\theta$ с тези по-надолу. Доколкото виждам, никъде не използваш конкретните $\sigma$ и $\tau$, така че може просто да не разписваш $\theta$}.
    is a subtype assertion satisfying the conditions from \cref{def:subadd}
    and $\langle T', \less' \rangle = \langle T, \less \rangle \tadd \theta$, then
...
\begin{proof}
    ...
    $\suptyper{\xi}$ are injections.\marginpar{\scriptsize и защото range-овете им не се пресичат}
    ...
    \begin{itemize}
        \item for each special $\tau$, $\squash_\theta(\tau)$ is a minimal element in $T$ such that
            $\tau \less' \squash_\theta(\tau)$:\marginpar{\scriptsize minimal и (minimal \ldots such that \ldots) са различни свойства!}
            ...
            in $T$ such that $\tau \less' \sigma$, therefore it is minimal.\marginpar{\scriptsize ако тук не напишеш ``\ldots minimal with this property'', не мисля, че ще е вярно}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item for $T_{i + 1}$, inductively we have that $T_i \less_i T$.\marginpar{\scriptsize тук може би искаш да кажеш, че $T_i \tless T$? Нещо не разбирам означенията\ldots като ги обсъдим, ще прегледам пак.}
...
Although terms returned by $\mcf{tag}_{\Gamma}$ are well-typed (the type of
