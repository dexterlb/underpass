...
\begin{defn}
    ...
    \begin{itemize}
        ...
        \item a mapping $\varphi : T \rightarrow \tclos{T}$\fixednote{\scriptsize тук означението $\pfun$ се появява за пръв път без да е обяснено, не мисля че може да се счита стандартно означение на частична функция.}
...
}\fixednote{\scriptsize Тази фигура няма ли да се опрости, ако махнеш $\varepsilon$-преходите?}

We will informally assume that term manipulations don't alter semantics\footnote{
    by much} (in a very broad sense).

This ``type-tagged term'' construction \fixednote{\scriptsize моля поясни: tagging? manipulation? propagation?} does not need to include casts, since we are storing
...
\begin{defn}
    ...
    The set of type-tagged $\lambda$-terms $\ttlambda{T}{C}$ is defined inductively\footnote{
        ...
        don't require here).
    }\fixednote{\scriptsize стандартната нотация за типове по термовете май е $M^\tau$. Ако нарочно въвеждаш различна нотация, за да разграничиш type-tagging от експлицитни системи стил Church, добре е да го кажеш явно.}:
    \begin{itemize}
        \item Constant:
...
describe a universal way to propagate information through those types.\fixednote{\scriptsize няма ли да покажеш, че термовете, които се връщат от $\mcf{untag}$ са коректно типизирани?}
