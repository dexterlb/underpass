...
\begin{proof}
    ...
    Let $\eta$ be a meet of $\sigma$ and $\tau$.
    \[
        \eta \less \sigma, \eta \less \tau,
            \forall \nu \in \tclos{T} (\nu \less \sigma, \nu \less \tau \implies \nu \less \eta)
    \]
...
\begin{proof}
    ...
    Now to check the conditions for $\squash_{\theta}$
    ...
    The only thing left is to check tht $\squash_{\theta}$ returns maximal
...
system into a basic type system and constructs a mapping which does this in a
...
We will then define the functions $\mcf{tag}$ and $\mcf{untag}$ which allow us
to convert typed $\lambda$-terms into type-tagged $\lambda$-terms and vice
versa, and the function $\mcf{propagate}$ which propagates type information
throughout the term. The main goal is to be able to $\mcf{tag}$ a term,
perform rewriting passes which depend on type information, then $\mcf{propagate}$
...
\cref{fig:propagation}.

\begin{figure}[htbp!]
    \includegraphics[width=0.6\textwidth]{propagation.eps}
...
\begin{defn}
    ...
    $\mcf{up}_{\unify}$ which maps a type-tagged term into a new type-tagged term and
    an inferred context for its free variables, and $\mcf{down}_{\unify}$ which maps
