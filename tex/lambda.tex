...
\begin{prop}
    If $\unify$ is a monotonic unifier for $T$ with a partial order relation
    $\less$ that has no infinite decreasing sequences, then
    $\mcf{propagate}_{\unify}$ reaches a fixed point on every term, i.e.
    \[ \forall P \in \fancylambda{T}{C} \quad \exists n \in \mathbb{N} \quad
