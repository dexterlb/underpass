...
\begin{proof}
    Without loss of generality\footnote{The argument for general elements is dual to the one for special
        elements.},
    let $\tau$ be a special element in $T \setminus S$.
    Because of the ``exclusive or'' in \cref{def:tless}, this means that
    $\exists \sigma \in S (\tau \less' \sigma)$ and
    $\neg \exists \sigma \in S (\sigma \less' \tau)$. Thus, for every
    $\sigma \in S$ that $\tau$ is comparable with, $\tau \less' \sigma$.
    \fixednote{има разминаване с дефиницията на ``special'': $(\exists \sigma \in S)(\tau \less'\sigma)$. Не трябва ли да е ``for some''? Изглежда, че липсва една стъпка, която се позовава на $\xor$ в дефиницията на $\tless$, за да заключи, че от $\exists$ следва $\forall$.}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item for $T_{i + 1}$, we will define\fixednote{тук може би искаш да кажеш, че $T_i \tless T$? Нещо не разбирам означенията\ldots като ги обсъдим, ще прегледам пак.}
...
process until\footnote{
    We will prove that type propagation always terminates. The termination of
    rewriting, however, depends on the nature of the rewriter itself,
    and will just be discussed briefly.
} \fixednote{възможно ли е този процес да зацикли?}
it no longer produces a different term, and then $\mcf{untag}$ging
...
For convenience, we
will also define the notion of \emph{length} on type-tagged terms:
\begin{defn}
    \[
        \termlen{\ttt{A}{\tau}} \defeq
        \begin{cases*}
            1 , & $A = c \in C$, \\
            1 , & $A = v \in \lvars$, \\
            \termlen{P} + \termlen{Q} + 1, & $A = (PQ)$, \\
            \termlen{P} + 1 , & $A = (\lambda v \abstr P)$. \\
        \end{cases*}
    \]
\end{defn}
...
\begin{defn}\label{def:mono}
    ...
    \emph{monotonic} with respect to the partial order
    $\less \subseteq \tclos{T} \times \tclos{T}$ when:
    ...
    \begin{itemize}
        \item Constant:
            \cenderiv{
                \hypo{a, b \in C}
                \hypo{\tau \less \sigma}
                \infer2{\ttt{a}{\tau} \termless{T}{C} \ttt{b}{\sigma}}
            }
        \item Variable:
            \cenderiv{
                \hypo{v, w \in \lvars}
                \hypo{\sigma, \tau \in \tclos{T}}
                \hypo{\tau \less \sigma}
                \infer3{\ttt{v}{\tau} \termless{T}{C} \ttt{w}{\sigma}}
            }
        \item Application:
            \cenderiv{
                \hypo{A_1, A_2, B_1, B_2 \in \ttlambda{T}{C}}
                \hypo{A_1 \termless{T}{C} A_2}
                \hypo{B_1 \termless{T}{C} B_2}
                \hypo{\tau \less \sigma}
                \infer4{\ttt{(A_1 B_1)}{\tau} \termless{T}{C} \ttt{(A_2 B_2)}{\sigma}}
            }
        \item Abstraction:
            \cenderiv{
                \hypo{v, w \in \lvars}
                \hypo{A, B \in \ttlambda{T}{C}}
                \hypo{A \termless{T}{C} B}
                \hypo{\tau \less \sigma}
                \infer4{\ttt{(\lambda v \abstr A)}{\tau} \less \ttt{(\lambda w \abstr B)}{\sigma}}
            }
    \end{itemize}
\end{defn}
...
\begin{lemma}\label{lemma:upmono}
    ...
    $P \in \ttlambda{T}{C}$,
    \[ (P', \Gamma) = \mcf{up}_{\unify}(P) \implies P' \termless{T}{C} P. \]
\end{lemma}
\begin{proof}
    Let $(\ttt{A'}{\tau'}, \Gamma) = \mcf{up}_{\unify}(\ttt{A}{\tau})$.
    ...
    \begin{itemize}
        \item $A = c \in C$:
            \[ \ttt{A'}{\tau'} =
                \underbrace{\ttt{c}{\tau} \termless{T}{C} \ttt{c}{\tau}}_{\text{since } \tau \less \tau}
                = \ttt{A}{\tau} \]
        \item $A = v \in \lvars$:
            \[ \ttt{A'}{\tau'} =
                \underbrace{\ttt{v}{\tau} \termless{T}{C} \ttt{v}{\tau}}_{\text{since } \tau \less \tau}
                = \ttt{A}{\tau} \]
        \item $A = (\lambda x \abstr P), \tau = (\eta \tot \sigma),
                (\ttt{M}{\sigma'}, \Gamma) = \mcf{up}_{\unify}(P),
                \Gamma' = \Gamma \setminus (x : \eta'), (x : \eta') \in \Gamma$:
            Inductively, we have that $\ttt{M}{\sigma'} \termless{T}{C} P$.
            We also know that $\tau' = (\eta \tot \sigma') \unify \tau \less \tau$.
            Thus,
            \[ \ttt{A'}{\tau'} =
                \ttt{\lambda x \abstr \ttt{M}{\sigma'}}{\tau'}
                \termless{T}{C}
                \ttt{(\lambda x \abstr P)}{\tau} =
                \ttt{A}{\tau}.
            \]
        \item $A = (PQ), (P', \Gamma_P) = \mcf{up}_{\unify}(P),
            (Q', \Gamma_Q) = \mcf{up}_{\unify}(Q),
            P' = \ttt{M}{\sigma' \tot \sigma''}$

            Inductively, $P' \termless{T}{C} P$ and $Q' \termless{T}{C} Q$.
            Since also $\sigma'' \unify \tau \less \tau$, we have:

            \[ \ttt{A'}{\tau'} =
                \ttt{P' Q'}{\sigma'' \unify \tau}
                \termless{T}{C}
                \ttt{P Q}{\tau} =
                \ttt{A}{\tau}.
            \]
...
\end{lemma}
\begin{proof}
    Let $\ttt{A'}{\tau} = \mcf{down}_{\unify}(\ttt{A}{\tau}, \sigma, \Gamma)$.
    We will prove the lemma inductively:

    \begin{itemize}
        \item $A = c \in C$:
            \[ \ttt{A'}{\tau'} =
                \underbrace{\ttt{c}{\tau \unify \sigma}
                \termless{T}{C} \ttt{c}{\tau}}_{\text{since } (\tau \unify \sigma) \less \tau}
                = \ttt{A}{\tau} \]
        \item $A = v \in \lvars, (v : \tau') \in \Gamma$:
            \[ \ttt{A'}{\tau'} =
                \underbrace{\ttt{v}{\tau \unify \sigma \unify \tau'}
                \termless{T}{C} \ttt{v}{\tau}}_{\text{since } (\tau \unify \tau' \unify \sigma) \less \tau}
                = \ttt{A}{\tau} \]
        \item $A = (\lambda x \abstr P), \tau'' = \tau \unify \sigma,
            \tau'' = (\sigma \tot \eta), P' = \mcf{down}_{\unify}(P, \eta, \Gamma \circ (x : \sigma))$:
            Inductively, $P' \termless{T}{C} P$. Furthermore,
            $(\tau \unify \sigma) \less \tau$. Thus,
            \[ \ttt{A'}{\tau'} =
                \ttt{(\lambda x \abstr P')}{\tau \unify \sigma}
                \termless{T}{C}
                \ttt{(\lambda x \abstr P)}{\tau}
                = \ttt{A}{\tau}
            \]
        \item $A = (PQ), P = \ttt{M}{\nu \tot \eta}, Q = \ttt{N}{\nu'},
                \tau' = (\sigma \unify \tau), \nu'' = (\nu' \unify \nu),
                P' = \mcf{down}_{\unify}(P, (\nu' \tot \sigma'), \Gamma),
                Q' = \mcf{down}_{\unify}(Q, \nu'', \Gamma)$:

            Inductively, $P' \termless{T}{C} P$ and $Q' \termless{T}{C} Q$.
            Since also $\tau' = \tau \unify \sigma \less \tau$, we have:

            \[ \ttt{A'}{\tau'} =
                \ttt{P' Q'}{\tau'} \termless{T}{C}
                \ttt{P Q}{\tau} =
                \ttt{A}{\tau}
            \]
    \end{itemize}
...
\begin{lemma}\label{lemma:termpartial}
    If $T$ is a set of type symbols and $\less \subseteq \tclos{T} \times \tclos{T}$
    is a partial order relation on $\tclos{T}$, the relation
    $\termless{T}{C}$
    is also a partial order relation.
\end{lemma}
\begin{proof}
    This lemma can be proven by a simple inductive argument.
\end{proof}

\begin{lemma}\label{lemma:rellen}
    If $T$ is a set of type symbols, $\less \subseteq \tclos{T} \times \tclos{T}$
    is a partial order relation on $\tclos{T}$ and
    $\ttt{M}{\sigma} \termless{T}{C} \ttt{N}{\tau}$, then:
    \begin{equation}\label{cond:rellen}
        \termlen{M} = \termlen{N}
    \end{equation}
    \begin{equation}\label{cond:reldir}
        \sigma \less \tau
    \end{equation}
\end{lemma}
\begin{proof}
    \cref{cond:reldir} stems trivially from the definition of $\termless{T}{C}$.

    \cref{cond:rellen} can be proven by induction over the same definition,
    since the terms on the left and right of $\termless{T}{C}$ always
    have the same syntactic structure.
\end{proof}

\begin{lemma}\label{lemma:lenwellfounded}
    If $T$ is a set of type symbols, $\less \subseteq \tclos{T} \times \tclos{T}$
    is a \emph{well-founded} partial order relation on $\tclos{T}$ and
    $n \in \mathbb{N}$, then every nonempty subset of $\ttlambda{T}{C}$ of
    type-tagged terms of length $n$ has a minimal element with respect to
    $\termless{T}{C}$.
\end{lemma}
\begin{proof}
    We will prove this lemma by strong induction over the term length $n$.

    First, for $n = 1$, we have that any nonempty subset of $\ttlambda{T}{C}$
    contains only Constants or Variables. Each constant or variable is only
    related to itself, so the proof is trivial.

    Now, we will prove the lemma for $n > 1$:

    Let $S \subseteq \ttlambda{T}{C}$ be a nonempty set of terms of length $n$.

    Furthermore, let
    $\tau_0$ be a minimal element
    of the set $\{ \tau \mid \ttt{M}{\tau} \in S \}$
    with respect to $\less$.

    We know that if the set $S' = \{ \ttt{M}{\tau} \mid \tau = \tau_0 \}$
    has a minimum, it is also the
    minimum of $S$, since the contrary would contradict the choice of
    $\tau_0$ due to \cref{cond:reldir}.

    Suppose that $S'$ has no minimum, i.e. there exists an infinite
    strictly-decreasing chain:
    \[ ... \termlessne{T}{C}
       \ttt{M_3}{\tau_0} \termlessne{T}{C} \ttt{M_2}{\tau_0} \termlessne{T}{C}
       \ttt{M_1}{\tau_0} \termlessne{T}{C} \ttt{M_0}{\tau_0} \]
    such that for every $i \in \mathbb{N}, \termlen{\ttt{M_i}{\tau_0}} = n$.

    Furthermore, the definition of $\termless{T}{C}$ implies that all terms
    in the chain have the same syntactic structure:
    \begin{itemize}
        \item For every $i \in \mathbb{N}$, $M_i$ is an Application, i.e.
            $M_i = P_i Q_i$:

            We know that $\termlen{P_i} \le \termlen{M_i}$ and
            $\termlen{Q_i} \le \termlen{M_i}$ since
            $\termlen{M_i} = \termlen{P_i} + \termlen{Q_i} + 1$.

            Inductively, the sets $\{ P_i \mid i \in \mathbb{N} \}$ and
            $\{ Q_j \mid j \in \mathbb{N} \}$ have minimal elements
            $P_i$ and $Q_j$ for some fixed $i$ and $j$. Without loss of
            generality, let $i \ge j$.

            We have $\ttt{M_{i + 1}}{\tau_0} \termless{T}{C} \ttt{M_i}{\tau_0}$,
            i.e. $\ttt{P_{i + 1} Q_{i + 1}}{\tau_0} \termless{T}{C}
            \ttt{P_i Q_i}{\tau_0}$. The definition of $\termless{T}{C}$
            implies that $\ttt{P_{i + 1}}{\tau_0} \termless{T}{C} \ttt{P_i}{\tau_0}$:
            thus, since $P_i$ is minimal,
            $P_{i + 1} = P_i$. Similarly, $Q_{j + 1} = Q_j = Q_{i} = Q_{i + 1}$.

            Therefore, the elements $\ttt{P_{i + 1} Q_{i + 1}}{\tau_0}$ and
            $\ttt{P_i Q_i}{\tau_0}$ are equal, which contradicts our supposition.

        \item For every $i \in \mathbb{N}$, $M_i$ is an Abstraction:
...
\end{lemma}
\begin{proof}
    The fact that $\termless{T}{C}$ is a partial order is given by \cref{lemma:termpartial}.

    To prove well-foundedness, take a nonempty set
    $S \subseteq \ttlambda{T}{C}$ and fix
    $M \in S$. Let $n = \termlen{M}$. We can isolate
    $S_n = \{ N \mid \termlen{N} = n \}$. By \cref{lemma:lenwellfounded},
    $S_n$ has a minimal element. By \cref{cond:rellen} we know that this
    minimal element is not related to any element in $S \setminus S_n$,
    therefore it is also minimal in $S$.
\end{proof}
...
In practice, the $\mcf{propagate}$ step shall be combined with a step which manipulates
the term for optimisation purposes. This is illustrated in \cref{sec:optimisation}.

\fixednote{липсва стъпката за манипулиране на терм}
\end{document}
