...
\begin{property}\label{prop:squash:unique}
    If there is a squash function $\squash: T \rightarrow S$ for
    $\langle T, \less' \rangle \tless \langle S, \less \rangle$,
    there is no other squash function for
    $\langle T, \less' \rangle \tless \langle S, \less \rangle$.
\end{property}
\begin{proof}
    Let $\squash' : T \rightarrow S$ be a squash function for
    $\langle T, \less' \rangle \tless \langle S, \less \rangle$ such that
    $\squash(\sigma) \not\eq \squash'(\sigma)$ for a fixed $\sigma \in T$.
...
\begin{lemma}
    ...
    is a subtype assertion satisfying the conditions from \cref{def:subadd}
    and $\langle T', \less' \rangle = \langle T, \less \rangle \tadd \theta$, then
    there is a squash function $\squash_{\theta} : T' \rightarrow T$.
\end{lemma}
\begin{proof}
    Let $\subtyper{\xi}$ and $\suptyper{\xi}$ be the injections from \cref{def:subadd}.
        ...
        \item It preserves the subtyping relation:
            ...
            \begin{itemize}
                ...
                \item $\sigma \in (T' \setminus T), \tau \in (T' \setminus T)$:
                    ...
                    $\sigma \less' \sigma'$ \fixednote{\scriptsize че $\sigma'$ е единствен е ясно, но откъде следва че релацията е $\sigma\less'\sigma'$, а не $\sigma'\less'\sigma$?} and $\tau'$ is the only element in $T$
