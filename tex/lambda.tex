...
\greenbox{
    ...
    will be written as ``$T \less S$'' and $\less$ will be used in the place
    of $\less'$ freely.
    
    This is unambigous because $\less'$ acts the same way as $\less$ on $S$,
    so we can assume to use $\less'$ in both cases.
}
...
\begin{defn}
    ...
    \begin{itemize}
        \item for each $\tau$, $\squash(\tau)$ is a minimal element in $S$ such that
            $\tau \less \squash(\tau)$.
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item for $T_{i + 1}$, inductively we have that $T_i \less_i T$.
            ...
            \begin{enumerate}
                \item For each $\tau \in T_{i + 1}$, $\squash_{i + 1}(\tau)$ is a unique
                    minimal element in $T$ such that $\tau \less \squash_{i + 1}(\tau)$.
                   
                    Fix an arbitrary $\tau$ and let 
