...
\example{
    ...
    in this way. This can be remedied by setting $small \catdefsep Set \rci{\modr} Set$
...
used in a category.
\begin{defn}
    Let $fv: C(\tau) \rightarrow 2^{\cvars}$ be defined as:
    \[
        fv(Z) =
        \begin{cases*}
            \varnothing, & $Z \in \tau$ \\
...
\example{
    ...
    \gramshort{
        ...
        \gramrow{from}{DistCheck \lc Dist \rc Set}{
            ...
            \gramrow{}{}{]}
        \gramrow{north \sq of}{NorthCheck \lc Dist \rc Set}{
            \lambda c {:} Set, d {:} Dist \abstr NorthCheck [}
            \gramrow{}{}{ \quad \lambda f {:} (Dist \tot Set \tot Set) \abstr f \app d \app c }
            \gramrow{}{}{]}
    }
