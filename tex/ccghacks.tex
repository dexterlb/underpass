...
\marginpar{\scriptsize тук е добре да се сложи едно пояснително изречение или подчинено изречение}

The simplest method to alleviate this is to allow arbitrary rule restrictions
depending\marginpar{\scriptsize depending on what?}, essentially creating a new formalism for each new target language.
...
and introduce variable substitution over categories.\marginpar{\scriptsize това изречение е изостанало отнякъде}
...
used in a category.\marginpar{\scriptsize Предложение: For convenience, we will use the same notation as the function returning the free variables of $\lambda$-terms.}
\begin{defn}
...
\marginpar{\scriptsize Формално погледнато, тук прилагаш субституция върху множество от термове. Също, може би е добре за пълнота да кажеш какво са $root$ и $crown$.}
