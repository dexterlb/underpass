...
\begin{enumerate}
    ...
    \item If $(X \rc Y', i, p) \in P, (Y'' \mci{1} Z_1 \mci{2} Z_2 ... \mci{m} Z_m, p + 1, j) \in P$
        ...
        \marginpar{\scriptsize Тук не трябва ли да е $(\xi'(X) \mci{1} \xi''(Z_1) \mci{2} \xi''(Z_2) ... \mci{m} \xi''(Z_m), i, j)\in P$ заради лявата асоциативност?}
...
Now, to use the unifier for parsing we must extend it over complex categories.\marginpar{\scriptsize тук не се разбира какво имаш предвид}
...
\begin{defn}\label{def:rel:uniform}
    ...
    inductively as follows:\marginpar{\scriptsize за индуктивни дефиниции досега си използвал стила със клаузи, защо не го ползваш и тук?}
    \begin{align*}
        \hat{U}(X, Y) \defiff & \\
...
relation can be used as an equification relation.\marginpar{\scriptsize ако човек не е чел раздел \cref{sec:lambda}, струва ми се, че много трудно ще се ориентира тук...}
...
\begin{defn}
    ...
    inductively as follows:\marginpar{\scriptsize също препоръчвам дефиниция с клаузи}
    \begin{align*}
...
\begin{example}
    ...
    \end{align*}
    and looks like this:\marginpar{\scriptsize запетайка в началото на реда изглежда странно}
    \gramshort{
    ...
    (they are paired with different $\lambda$-terms)\marginpar{не става ясно: кои са they и кои са различните $\lambda$-термове?}.
