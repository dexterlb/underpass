...
\begin{center}
    \tree{.{$\\subst{X}{\alpha}{Y}$} \edge[very thick]; {$X$} }
\end{center}
...
\begin{center}
    \tree{.{$\\alpha \rc \lp \alpha \lc X \rp$} \edge[very thick]; {$X$} }
        ( Forward type-raising )
    \tree{.{$\alpha \lc \lp \alpha \rc X \rp$} \edge[very thick]; {$X$} }
...
performs variable substitution while unifying) and embedding regular expressions.
